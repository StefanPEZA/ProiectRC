...
songInfoWidget::songInfoWidget(int *_sv_sock, int _acc_type, int _place, int _song_id, QString _name, QString _author, QString _desc,
   ...
   ui(new Ui::songInfoWidget)
{
   ...
   ui->songDesc->setHtml(desc + "<br><br>Genuri muzicale: " + genres +
                         "<br>Link: <a style = \"text-decoration:none;color:#F2AA4C\" href = " + link + ">" + link + "</a>");
...
int songInfoWidget::SendRequestToServer(char *request, char *response)
{
   ...
   {
      QMessageBox::information(this, "Ceva nu e bine!", "Conectiunea la server a fost pierduta! (" + QString(strerror(errno)) + ")");
      return(0);
   }
...
{
   if (QMessageBox::question(this, "Retragi votul?", "Sigur vrei sa-ti retragi votul de la aceasta melodie?", QMessageBox::Yes | QMessageBox::No)
       != QMessageBox::Yes)
   {
      return;
   }
   ...
   {
      QMessageBox::information(this, "Nu-ti mai place?", "Ti-ai retras votul cu succes de la melodia: " + name + " - " + author);
   }
}

void songInfoWidget::on_songDelete_clicked()
{
   if (QMessageBox::question(this, "Esti sigur?", "Sigur vrei sa stergi melodia din top?", QMessageBox::Yes | QMessageBox::No)
       != QMessageBox::Yes)
   {
      return;
   }
   ...
   {
      QMessageBox::information(this, "Succes, din pacate!", "Stergerea melodiei a avut loc cu succes! " + name + " - " + author);
   }
}
