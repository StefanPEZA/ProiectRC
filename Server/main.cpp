...
int            client_count = 0;                   /* numarul de clienti autentificati pe server */
list <string> *client_names = new list <string>(); /* retine numele de utilizator a fiecarui client autentificat*/

void *serverthread(void *client_socket);           /* functia thread care se ocupa de un anumit client   */
   ...
   while (1)
   {
      ...
      if (!has_access && strchr(request, '|'))
      {
         ...
         else if (!strcmp(token, "REGISTER") && !has_access)
         {
            ...
            if (ok)
            {
               has_access = TryClientRegister(&usertype, tnumber, username, password, first_name, last_name, acc_type, response, &DB_handler);
            }
            else
            {
...
bool HandleRequest(int *usertype, int tnumber, const char *username, char *request, char *response, DataHandler *DB_handler)
{
   char *token = strtok(request, "|");

   if (!strcmp(token, "GET_TOP"))
   {
      char *genre = strtok(NULL, "|");
      printf("[SERVER]>>Solicitare de la %s (%d): trimite topul cu melodii (genul : %s)\n", username, tnumber, genre);
      fflush(stdout);
...
bool TryClientLogin(int *usertype, int thread_number, const char *username, const char *password, char *response, DataHandler *database)
{
...
bool TryClientRegister(int *usertype, int thread_number, const char *username, const char *passsword, const char *first_name,
                       const char *last_name, int acount_type, char *response, DataHandler *database)
{
   if (!userAlreadyConnected(username))
   {
      ...
      fflush(stdout);
      if ((*database).TryRegister(usertype, username, passsword, first_name, last_name, acount_type, response))
      {
         //adaugarea clientului la lista clientilor deja autentificati pe server
         pthread_mutex_lock(&mut);
         client_count++;
